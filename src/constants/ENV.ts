export const DEV_MODE = import.meta.env.MODE === 'development';
export const MONGO_HOST = import.meta.env.VITE_MONGO_HOST ?? 'localhost';
export const MONGO_PORT = import.meta.env.VITE_MONGO_PORT ?? '27019';
export const MONGO_DBNAME = import.meta.env.VITE_MONGO_DBNAME;
export const REDIS_HOST = import.meta.env.VITE_REDIS_HOST ?? 'localhost';
export const REDIS_PORT = import.meta.env.VITE_REDIS_PORT ?? '6379';
export const REDIS_PREFIX = import.meta.env.VITE_REDIS_PREFIX ?? '';
export const OAUTH_REDIRECT_URL = import.meta.env.VITE_OAUTH_REDIRECT_URL;
export const DOMAIN = import.meta.env.VITE_DOMAIN;
